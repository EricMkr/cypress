...
describe("Landing Page Form Tests", () => {
  ...
  // Nettoie le fichier data.json avant tous les tests
  before(() => {
    cy.resetDataFile();
  });
...
describe("Loader and Animation", () => {
  ...
  it("Affiche le loader pendant l’envoi et l’animation check après", () => {
    cy.intercept("POST", "/save", (req) => {
      return new Promise((resolve) => setTimeout(() => resolve({ statusCode: 200, body: "OK" }), 1000));
    }).as("postWithDelay");
    ...
    cy.get("button#submitBtn svg.checkmark").should("exist");
  });
});
